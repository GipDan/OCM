...
def main() -> None:
    ...
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(
        ["录入数据", "手动训练", "导出 CSV", "模型干预", "推理试算", "数据管理"]
    )
    ...
    with tab6:
        st.markdown("预览、修改、删除当前库中 **`records`** 表的样本（可按算子/设备筛选）。")
        fc1, fc2, fc3 = st.columns([2, 2, 1])
        with fc1:
            mgmt_op = st.text_input("筛选 op_name（留空=全部）", key="mgmt_f_op")
        with fc2:
            mgmt_dev = st.text_input("筛选 device（留空=全部）", key="mgmt_f_dev")
        with fc3:
            mgmt_limit = st.number_input(
                "最多条数", min_value=10, max_value=5000, value=500, step=10, key="mgmt_limit"
            )

        fop = mgmt_op.strip() or None
